...
class Distance2Self(object):
    ...
    def __init__(self, _matrix, trie=None, saveTrieFile=False):
        ...
        # Where should files reside ?/home/theresa/dist2self/fred2/
        this_dir = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
        self.__externalPathDistanceCalculator = os.path.join(this_dir, 'src', 'compute_distances_ivac')
        self.__externalPathTrieGenerator =  os.path.join(this_dir, 'src','get_TrieArray')
    ...
    def convert_alleles(self, alleles):
        ...
        :rtype: list(str)
        """
        return ["HLA_%s%s_%s" % (a.locus, a.supertype, a.subtype) for a in alleles]
    ...
    def calculate_distances(self, peptides, alleles=None, hla_header="HLA", pep_header="neopeptide", pathToTrie=None, n=10):
        def __load_trie(al):
            current = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
            return os.path.join(current,"data","tries","proteome_thymusmax_"+al+".trie")
        ...
        for a in alleles_string.iterkeys():
            trie = __load_trie(a) if pathToTrie is None else pathToTrie
            method = "proteome-thymus" if pathToTrie is None else "custom"
            try:

                re = self.parse_external_result(
                    subprocess.check_output(cmd % (self.__matrix.path_to_matrix_file, trie, tmpFile.name, n),
                                            shell=True))

                for k, vs in re.iteritems():
                    results.setdefault(pep_header, []).append(k)
                    results.setdefault(hla_header, []).append("HLA-" + alleles_string[a].name)
                ...
                logging.warning("Could not make distance calculation for HLA allele {allele}".format(
                    allele=alleles_string[a]))
